...
int main(int argc, char* argv[])
{
	...
	for (int matchInd = 0; matchInd < matches.size(); matchInd++)
	{
		...
		float angleDelta = (objectInScene.angle - object.angle) * (float)CV_PI / 180.f;
		float scaleDelta = objectInScene.size / object.size;

		int radius = cvRound(dist * scaleDelta);

		Point orient(cvRound(cos(angle(center - object.pt) + angleDelta) * radius), cvRound(sin(angle(center - object.pt) + angleDelta) * radius));
	...
	vector<Point2f> keypointsCrds1(matches.size()), keypointsCrds2(matches.size());

	for (int matchInd = 0; matchInd < matches.size(); matchInd++) 
	{
		keypointsCrds1[matchInd] = keypoints1[matches[matchInd].queryIdx].pt;
		keypointsCrds2[matchInd] = keypoints2[matches[matchInd].trainIdx].pt;
	}

	Mat hommography = findHomography(keypointsCrds1, keypointsCrds2, CV_RANSAC), warp;
	warpPerspective(image, warp, hommography, scene.size());
