...
int main(int argc, char* argv[])
{
	...
	for (int matchInd = 0; matchInd < matches.size(); matchInd++)
	{
		...
		float deltaAngle = (objectInScene.angle - object.angle) * (float)CV_PI / 180.f;
		float deltaScale = objectInScene.size / object.size;

		Point orient(int(cos(angle(center - object.pt) + deltaAngle) * norm(center - object.pt) * (deltaScale)),
			int(sin(angle(center - object.pt) + deltaAngle) * norm(center - object.pt) * (deltaScale)));

		Point centerInScene = Point((int)objectInScene.pt.x, (int)objectInScene.pt.y) + orient;
	...
	Mat homoMatrix = findHomography(goodKeypoint1, goodKeypoint2, CV_RANSAC);
	Mat warpImg;
	warpPerspective(image, warpImg, homoMatrix, scene.size());
	imshow("Homography", warpImg);
	waitKey(0);
