...
int main(int argc, char* argv[])
{
	...
	for (size_t matchInd = 0; matchInd < matches.size(); matchInd++)
	{
		...
		float angleToCenter = angle(center - obj.pt);
		...
		Point centerInScene = Point((int)objInScene.pt.x, (int)objInScene.pt.y) + orient;
	...
	Mat hommat = findHomography(goodKeypoint1, goodKeypoint2, CV_RANSAC);
	Mat warpImage;
