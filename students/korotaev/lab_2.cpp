...
int main(int argc, char* argv[])
{
	...
	{
		KeyPoint point = keypoints1[matches[matchInd].queryIdx];
		KeyPoint pointInScene = keypoints2[matches[matchInd].trainIdx];

		float r = (float) norm(center - point.pt);
		float alpha = atan((center - point.pt).y / (center - point.pt).x);

		float delta_a = (pointInScene.angle - point.angle) * CV_PI / 180.0f;
		float delta_scale = pointInScene.size / point.size;

		int r2 = r * delta_scale;

		Point shift( (cos(alpha + delta_a) * r2), (sin(alpha + delta_a) * r2) );

		Point centerInScene = Point(pointInScene.pt.x, pointInScene.pt.y) + shift;
	...
	for (int matchInd = 0; matchInd < matches.size(); matchInd++) {
		srcPoints[matchInd] = keypoints1[matches[matchInd].queryIdx].pt;
		dstPoints[matchInd] = keypoints2[matches[matchInd].trainIdx].pt;
	}
	Mat hommat = findHomography(srcPoints, dstPoints, CV_RANSAC, 3);
	Mat warp;
	warpPerspective(image, warp, hommat, scene.size());
