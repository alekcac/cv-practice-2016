...
float angle(Point2f  &Vertex)
  {
	  float Gip = sqrt(Vertex.x*Vertex.x +Vertex.y*Vertex.y);
	  return acos(Vertex.x/Gip);
  }
int main(int argc, char* argv[])
{
  ...
  {
	  KeyPoint object = keypoints1[matches[matchInd].queryIdx];
	  KeyPoint  objectInScene = keypoints2[matches[matchInd].trainIdx];

	  float dist = (float)norm(center - object.pt);
  	
  
  	  float dAngle = (objectInScene.angle - object.angle) * (float)CV_PI / 180.0f;
  	  int rad = cvRound(dist * objectInScene.size / object.size);
	  int orientX = cvRound(rad*cos(angle(center - object.pt) + dAngle) );
	  int orientY = cvRound(rad*sin(angle(center - object.pt) + dAngle));
  	  Point centerInScene = Point((int)objectInScene.pt.x, (int)objectInScene.pt.y) + Point(orientX, orientY);
  	  hgt.at<int>(centerInScene.y / splitSet, centerInScene.x / splitSet) += 1;
  }
  Point maxP;
  ...
  vector<Point2f> goodKeypoint1(matches.size());
  vector<Point2f> goodKeypoint2(matches.size());
  for (size_t matchInd = 0; matchInd < matches.size(); matchInd++) {
      goodKeypoint1[matchInd] = keypoints1[matches[matchInd].queryIdx].pt;
      goodKeypoint2[matchInd] = keypoints2[matches[matchInd].trainIdx].pt;
  }
  Mat homographyMatrix = findHomography(goodKeypoint1, goodKeypoint2, CV_RANSAC);
  Mat warpImage;
  warpPerspective(image, warpImage, homographyMatrix, scene.size());
